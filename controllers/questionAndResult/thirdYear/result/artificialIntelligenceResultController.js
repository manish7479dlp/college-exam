const artificialIntelligenceResultModel = require("../../../../models/questionAndAnswer/thirdYear/result/artificialIntelligence");
const artificialIntelligenceQuestion = require("../../../../models/questionAndAnswer/thirdYear/question/artificialIntelligence");
//get all student result.
const getAllStudentResult = async (req, res) => {
    try {
        const response = await artificialIntelligenceResultModel.find();
...
const getParticularStudentResult = async (req , res) => {
    try {
        ...
        const response = await artificialIntelligenceResultModel.find({universityRoll});
        if (response.length === 0) {
            res.send({ status: false, message: "No Result Found." });
        } else {
            res.send({ status: true, response });
...
const calcMarks = async (marks) => {
    try {
        const response = await artificialIntelligenceQuestion.find();
        let result = 0;
...
const submitResult = async (req, res) => {
    try {
        ...
        const response = await artificialIntelligenceResultModel({ universityRoll, marks, name , department});
...
const deleteAllResult = async (req, res) => {
    try {
        const response = await artificialIntelligenceResultModel.deleteMany({
            department: "CSE",
        });
