const compilerDesignResultModel = require("../../../../models/questionAndAnswer/thirdYear/result/compilerDesign");
const compilerDesignQuestion = require("../../../../models/questionAndAnswer/thirdYear/question/compilerDesign");
//get all student result.
const getAllStudentResult = async (req, res) => {
    try {
        const response = await compilerDesignResultModel.find();
...
const getParticularStudentResult = async (req , res) => {
    try {
        ...
        const response = await compilerDesignResultModel.find({universityRoll});
        if (response.length === 0) {
            res.send({ status: false, message: "No Result Found." });
        } else {
            res.send({ status: true, response });
...
const calcMarks = async (marks) => {
    try {
        const response = await compilerDesignQuestion.find();
        let result = 0;
...
const submitResult = async (req, res) => {
    try {
        ...
        const response = await compilerDesignResultModel({ universityRoll, marks, name , department});
...
const deleteAllResult = async (req, res) => {
    try {
        const response = await compilerDesignResultModel.deleteMany({
            department: "CSE",
        });
