const softwareEngineeringResultModel = require("../../../../models/questionAndAnswer/thirdYear/result/softwareEngineering");
const softwareEngineeringQuestion = require("../../../../models/questionAndAnswer/thirdYear/question/softwareEngineering");
//get all student result.
const getAllStudentResult = async (req, res) => {
    try {
        const response = await softwareEngineeringResultModel.find();
...
const getParticularStudentResult = async (req , res) => {
    try {
        ...
        const response = await softwareEngineeringResultModel.find({universityRoll});
        if (response.length === 0) {
            res.send({ status: false, message: "No Result Found." });
        } else {
            res.send({ status: true, response });
...
const calcMarks = async (marks) => {
    try {
        const response = await softwareEngineeringQuestion.find();
        let result = 0;
...
const submitResult = async (req, res) => {
    try {
        ...
        const response = await softwareEngineeringResultModel({ universityRoll, marks, name , department});
...
const deleteAllResult = async (req, res) => {
    try {
        const response = await softwareEngineeringResultModel.deleteMany({
            department: "CSE",
        });
