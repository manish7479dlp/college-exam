const operatingSystemResultModel = require("../../../../models/questionAndAnswer/thirdYear/result/operatingSystem");
const operatingSystemQuestion = require("../../../../models/questionAndAnswer/thirdYear/question/operatingSystem");
//get all student result.
const getAllStudentResult = async (req, res) => {
    try {
        const response = await operatingSystemResultModel.find();
...
const getParticularStudentResult = async (req , res) => {
    try {
        ...
        const response = await operatingSystemResultModel.find({universityRoll});
        if (response.length === 0) {
            res.send({ status: false, message: "No Result Found." });
        } else {
            res.send({ status: true, response });
...
const calcMarks = async (marks) => {
    try {
        const response = await operatingSystemQuestion.find();
        let result = 0;
...
const submitResult = async (req, res) => {
    try {
        ...
        const response = await operatingSystemResultModel({ universityRoll, marks, name , department});
...
const deleteAllResult = async (req, res) => {
    try {
        const response = await operatingSystemResultModel.deleteMany({
            department: "CSE",
        });
