...
const softwareEngineeringModel = require("../../../../models/questionAndAnswer/thirdYear/question/softwareEngineering");
...
const registerQuestion = async (req, res) => {
    try {
        ...
        const validationCheck = await softwareEngineeringModel.findOne({ question });
        ...
        if (validationCheck) {
            ...
            if (question && opt1 && opt2 && opt3 && opt4 && answer) {
                const response = new softwareEngineeringModel({
                    question,
                    opt1,
                    opt2,
                    opt3,
                ...
                const totalQuestionCount = await softwareEngineeringModel.find().count();
                res.status(201).send({
                    status: true,
...
const getAllQuestion = async (req, res) => {
    try {
        const response = await softwareEngineeringModel.find();
...
const deleteParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await softwareEngineeringModel.findByIdAndDelete({ _id });
...
const updateParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await softwareEngineeringModel.findByIdAndUpdate(
            { _id },
            {
                $set: {
                    question,
...
const deleteAllQuestion = async (req, res) => {
    try {
        const response = await softwareEngineeringModel.deleteMany({ department: "CSE" });
