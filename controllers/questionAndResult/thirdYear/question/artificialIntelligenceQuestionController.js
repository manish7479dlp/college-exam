...
const artificialIntelligenceModel = require("../../../../models/questionAndAnswer/thirdYear/question/artificialIntelligence");
...
const registerQuestion = async (req, res) => {
    try {
        ...
        const validationCheck = await artificialIntelligenceModel.findOne({ question });
        ...
        if (validationCheck) {
            ...
            if (question && opt1 && opt2 && opt3 && opt4 && answer) {
                const response = new artificialIntelligenceModel({
                    question,
                    opt1,
                    opt2,
                    opt3,
                ...
                const totalQuestionCount = await artificialIntelligenceModel.find().count();
                res.status(201).send({
                    status: true,
...
const getAllQuestion = async (req, res) => {
    try {
        const response = await artificialIntelligenceModel.find();
...
const deleteParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await artificialIntelligenceModel.findByIdAndDelete({ _id });
...
const updateParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await artificialIntelligenceModel.findByIdAndUpdate(
            { _id },
            {
                $set: {
                    question,
...
const deleteAllQuestion = async (req, res) => {
    try {
        const response = await artificialIntelligenceModel.deleteMany({ department: "CSE" });
