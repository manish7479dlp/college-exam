...
const industrialManagementModel = require("../../../../models/questionAndAnswer/thirdYear/question/industrialManagement");
...
const registerQuestion = async (req, res) => {
    try {
        ...
        const validationCheck = await industrialManagementModel.findOne({ question });
        ...
        if (validationCheck) {
            ...
            if (question && opt1 && opt2 && opt3 && opt4 && answer) {
                const response = new industrialManagementModel({
                    question,
                    opt1,
                    opt2,
                    opt3,
                ...
                const totalQuestionCount = await industrialManagementModel.find().count();
                res.status(201).send({
                    status: true,
...
const getAllQuestion = async (req, res) => {
    try {
        const response = await industrialManagementModel.find();
...
const deleteParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await industrialManagementModel.findByIdAndDelete({ _id });
...
const updateParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await industrialManagementModel.findByIdAndUpdate(
            { _id },
            {
                $set: {
                    question,
...
const deleteAllQuestion = async (req, res) => {
    try {
        const response = await industrialManagementModel.deleteMany({ department: "CSE" });
