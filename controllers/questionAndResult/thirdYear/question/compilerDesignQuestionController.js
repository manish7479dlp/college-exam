...
const compilerDesignModel = require("../../../../models/questionAndAnswer/thirdYear/question/compilerDesign");
...
const registerQuestion = async (req, res) => {
    try {
        ...
        const validationCheck = await compilerDesignModel.findOne({ question });
        ...
        if (validationCheck) {
            ...
            if (question && opt1 && opt2 && opt3 && opt4 && answer) {
                const response = new compilerDesignModel({
                    question,
                    opt1,
                    opt2,
                    opt3,
                ...
                const totalQuestionCount = await compilerDesignModel.find().count();
                res.status(201).send({
                    status: true,
...
const getAllQuestion = async (req, res) => {
    try {
        const response = await compilerDesignModel.find();
...
const deleteParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await compilerDesignModel.findByIdAndDelete({ _id });
...
const updateParticularQuestion = async (req, res) => {
    try {
        ...
        const response = await compilerDesignModel.findByIdAndUpdate(
            { _id },
            {
                $set: {
                    question,
...
const deleteAllQuestion = async (req, res) => {
    try {
        const response = await compilerDesignModel.deleteMany({ department: "CSE" });
